...
class SpellImportSerializer(serializers.Serializer):
    ...
    spells = serializers.ListField(child=serializers.JSONField(), allow_empty=False)
    ...
    def validate_spells(self, value):
        ...
        for idx, spell_data in enumerate(value):
            ...
            if not has_name:
                raise serializers.ValidationError(f"Spell at index {idx} is missing required field: name")
            if not has_level:
                raise serializers.ValidationError(f"Spell at index {idx} is missing required field: level")
        return value
