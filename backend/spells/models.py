...
class Spell(models.Model):
    ...
    is_auto_hit = models.BooleanField(
        default=False, help_text="Spell automatically hits with no attack roll or saving throw (e.g. Magic Missile)."
    )
    ...
    classes = models.JSONField(default=list, blank=True, help_text="List of class names that can learn this spell.")

    # Gameplay tags — e.g. ["damage", "aoe", "utility", "crowd_control"]
    tags = models.JSONField(default=list, blank=True, help_text="Gameplay category tags for filtering and analysis.")
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="spells", null=True, blank=True
    )
...
class SpellParsingMetadata(models.Model):
    ...
    reviewed_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True, related_name="reviewed_spells"
    )
...
class SummonAttack(models.Model):
    ...
    def average_damage_at_level(self, slot_level: int) -> float:
        ...
        primary = self.dice_count * (self.die_size + 1) / 2 + self.flat_modifier + self.flat_per_level * slot_level
        secondary = (
            self.secondary_dice_count * (self.secondary_die_size + 1) / 2 + self.secondary_flat
