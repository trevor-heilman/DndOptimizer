...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--file", type=str, help="Path to a specific spell JSON file to import")
        parser.add_argument("--all", action="store_true", help="Import all default spell JSON files")
        parser.add_argument("--clear", action="store_true", help="Clear existing spells before importing")
    ...
    def handle(self, *args, **options):
        ...
        if options["file"]:
            ...
            raise CommandError("Please specify --file <path> or --all to import default files")
        ...
        for filepath in files_to_import:
            ...
            self.stdout.write(self.style.SUCCESS(f"  Imported {imported} spells"))
            if skipped > 0:
                self.stdout.write(
                    self.style.WARNING(f"  {skipped} spells already exist (name + source match) — skipped")
                )
            if errors > 0:
                self.stdout.write(self.style.WARNING(f"  {errors} spells had errors"))
    ...
    def _import_file(self, filepath: str) -> tuple:
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Failed to load file: {str(e)}"))
            return (0, 1, 0)
        ...
        for raw_spell in spells:
            try:
                with transaction.atomic():
                    ...
                    spell = SpellParsingService.create_spell_from_parsed_data(parsed, created_by=None)  # System import
                    ...
                    if parsed["requires_review"]:
                        self.stdout.write(
                            self.style.WARNING(
                                f'  {spell.name}: Low confidence ({parsed["confidence"]:.2f}), ' f"requires review"
                            )
                        )
                ...
                self.stdout.write(self.style.ERROR(f"  Failed to import {spell_name}: {str(e)}"))
