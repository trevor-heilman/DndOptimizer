...
class Command(BaseCommand):
        ...
        for cfg in AUTO_HIT_CORRECTIONS:
            ...
            try:
                ...
                self.stdout.write(self.style.WARNING(
                    f'  Multiple spells named "{name}" — updating all {spells.count()}.'
                ))
                for s in spells:
    ...
    def _apply(self, spell, cfg, dry_run):
        ...
        for field in ('is_auto_hit', 'number_of_attacks', 'upcast_attacks_increment',
                      'upcast_base_level', 'upcast_dice_increment'):
            ...
            for comp in spell.damage_components.all():
                if comp.flat_modifier != flat_mod:
                    fields_changed.append(
                        f'  component {comp.id} flat_modifier: {comp.flat_modifier} → {flat_mod}'
                    )
                    if not dry_run:
