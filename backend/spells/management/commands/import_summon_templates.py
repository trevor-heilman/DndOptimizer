...
DESCRIPTION_TO_SPELL: dict[str, str] = {
    desc: spell_name
    for spell_name in SUMMON_SPELL_META
    for desc in [f'Part of the {spell_name} spell']
}
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        creatures: list[dict[str, Any]] = [
            ...
            if isinstance(entry, dict) and entry.get('Actions')
        ]
        ...
        with transaction.atomic():
            for creature in creatures:
                ...
                if not spell_name:
                    self.stdout.write(
                        self.style.WARNING(f'  Skipping "{creature.get("Name")}" — unrecognised description: "{description}"')
                    )
                    skipped += 1
                    continue
                ...
                base_hp, hp_per_level, hp_base_level = _parse_hp(
                    hp_data.get('Value', 0), hp_data.get('Notes', '')
                )
                base_ac, ac_per_level = _parse_ac(
                    ac_data.get('Value', 0), ac_data.get('Notes', '')
                )
    ...
    def _dry_run(self, creatures: list[dict]) -> None:
        ...
        for creature in creatures:
            ...
            for action in creature.get('Actions', []):
                ...
                self.stdout.write(
                    f'         {action["Name"]}: {atype}, primary={primary}, secondary={secondary}'
                )
        self.stdout.write('\n--- END DRY RUN ---')
