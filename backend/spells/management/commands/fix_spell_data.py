...
COMPONENT_TIMING_CORRECTIONS = {
    ...
    "Acid Arrow": [
        ...
        {"dice_count": 2, "die_size": 4, "damage_type": "acid", "timing": "end_of_turn", "skip_first": False},
    ],
}
    ...
    def handle(self, *args, **options):
        ...
        for spell_name, corrections in COMPONENT_TIMING_CORRECTIONS.items():
            try:
                ...
                self.stdout.write(self.style.WARNING(f"  Multiple spells named {spell_name!r} — skipping timing fix"))
                continue
        ...
        for spell_name, remove_tags in TAGS_TO_REMOVE.items():
            ...
            for spell in qs:
                ...
                if new_tags != current_tags:
                    ...
                    self.stdout.write(
                        f'  {"[would fix]" if dry_run else "fixing"} tags: ' f"{spell_name!r} removing {removed!r}"
                    )
                    if not dry_run:
                        spell.tags = new_tags
                        spell.save()
        ...
        for spell_name, corrections in COMPONENT_ON_CRIT_EXTRA_CORRECTIONS.items():
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  Multiple spells named {spell_name!r} — skipping on_crit_extra fix")
                )
                continue

            for correction in corrections:
                dc_filter = {
                    k: correction[k] for k in ("dice_count", "die_size", "damage_type", "timing") if k in correction
                }
                new_val = correction["on_crit_extra"]
                skip_first = correction.get("skip_first", False)
                matching = list(spell.damage_components.filter(**dc_filter).order_by("created_at", "id"))
                candidates = matching[1:] if skip_first else matching
                for comp in candidates:
        ...
        for spell_name, corrections in COMPONENT_UPCAST_INCREMENT_CORRECTIONS.items():
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"  Multiple spells named {spell_name!r} — skipping upcast_dice_increment fix")
                )
                continue
