...
class SpellViewSet(viewsets.ModelViewSet):
    ...
    queryset = Spell.objects.all().prefetch_related(
        "damage_components", "parsing_metadata", "summon_templates__attacks"
    )
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated:
            ...
            queryset = queryset.filter(models.Q(is_custom=False) | models.Q(created_by=self.request.user))
        else:
    ...
    def spell_counts(self, request):
        ...
        if not request.user.is_authenticated:
            return Response({"system": 0, "imported": 0, "custom": 0}, status=status.HTTP_200_OK)
    ...
    def import_spells(self, request):
        ...
        if is_system and not request.user.is_staff:
            return Response(
                {"error": "Staff access required to import system spells"}, status=status.HTTP_403_FORBIDDEN
            )
        ...
        for spell_data in spells_data:
            try:
                ...
                failed_spells.append(
                    {"name": spell_data.get("name") or spell_data.get("Name", "Unknown"), "error": str(e)}
                )
    ...
    def bulk_delete(self, request):
        ...
        if not categories:
            return Response({"error": "Provide at least one category"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if "system" in categories:
            if not request.user.is_staff:
                return Response(
                    {"error": "Staff access required to delete system spells"}, status=status.HTTP_403_FORBIDDEN
                )
    ...
    def export_multiple(self, request):
        ...
        if not spell_ids:
            return Response({"error": "spell_ids required"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def needs_review(self, request):
        ...
        """
        queryset = (
            Spell.objects.filter(parsing_metadata__requires_review=True)
