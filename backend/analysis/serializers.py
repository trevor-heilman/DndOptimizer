...
class AnalysisContextSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if attrs.get("spell_slot_level", 1) < 1 or attrs.get("spell_slot_level", 1) > 9:
            raise serializers.ValidationError("Spell slot level must be between 1 and 9.")

        if attrs.get("number_of_targets", 1) < 1:
            raise serializers.ValidationError("Number of targets must be at least 1.")
...
class SpellComparisonRequestSerializer(ContextParametersMixin):
    ...
    number_of_targets_a = serializers.IntegerField(
        default=None, required=False, allow_null=True, min_value=1, max_value=20
    )
    number_of_targets_b = serializers.IntegerField(
        default=None, required=False, allow_null=True, min_value=1, max_value=20
    )
...
class SpellEfficiencyRequestSerializer(ContextParametersMixin):
    ...
    def validate(self, attrs):
        ...
        if attrs["min_slot_level"] > attrs["max_slot_level"]:
            raise serializers.ValidationError("min_slot_level cannot be greater than max_slot_level.")
