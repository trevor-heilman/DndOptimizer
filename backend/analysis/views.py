...
class AnalysisViewSet(viewsets.ViewSet):
    ...
    def analyze(self, request):
        ...
        data = serializer.validated_data
        spell = Spell.objects.prefetch_related("damage_components", "summon_templates__attacks").get(
            id=data["spell_id"]
        )
        ...
        # Create context
        context = AnalysisContext.create_from_data(data, user=request.user if request.user.is_authenticated else None)
...
class SpellComparisonViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = SpellComparison.objects.all().select_related("spell_a", "spell_b", "context")
