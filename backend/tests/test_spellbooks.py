...
from spellbooks.models import Character, Spellbook, PreparedSpell
...
def user(db):
    return User.objects.create_user(
        username='spellbookuser', email='sb@example.com', password='pass123'
    )
...
class TestCharacterCRUD:
    ...
    def test_partial_update_character(self, client, character):
        response = client.patch(
            f'/api/spellbooks/characters/{character.id}/', {'name': 'Olorin'}
        )
        assert response.status_code == status.HTTP_200_OK
...
class TestCharacterActions:
    ...
    def test_all_spells_with_prepared(self, client, character, spellbook, spell_fireball):
        PreparedSpell.objects.create(
            spellbook=spellbook, spell=spell_fireball, prepared=True
        )
        response = client.get(f'/api/spellbooks/characters/{character.id}/spells/')
        assert response.status_code == status.HTTP_200_OK
...
class TestCharacterComputedProperties:
    ...
    def test_wizard_max_prepared_spells_2014(self, client, user):
        char = Character.objects.create(
            name='Wizard 2014', character_class='wizard', character_level=5,
            spellcasting_ability_modifier=3, ruleset='2014', owner=user,
        )
    ...
    def test_wizard_max_prepared_spells_2024(self, client, user):
        char = Character.objects.create(
            name='Wizard 2024', character_class='wizard', character_level=5,
            spellcasting_ability_modifier=3, ruleset='2024', owner=user,
        )
        response = client.get(f'/api/spellbooks/characters/{char.id}/')
    ...
    def test_cleric_max_prepared_spells(self, client, user):
        char = Character.objects.create(
            name='Cleric', character_class='cleric', character_level=6,
            spellcasting_ability_modifier=4, ruleset='2014', owner=user,
        )
        response = client.get(f'/api/spellbooks/characters/{char.id}/')
    ...
    def test_sorcerer_has_no_max_prepared(self, client, user):
        char = Character.objects.create(
            name='Sorcerer', character_class='sorcerer', character_level=10,
            spellcasting_ability_modifier=5, ruleset='2014', owner=user,
        )
        response = client.get(f'/api/spellbooks/characters/{char.id}/')
    ...
    def test_proficiency_bonus_interpolation(self, client, user):
        char = Character.objects.create(
            name='Level 5 Char', character_class='wizard', character_level=5,
            spellcasting_ability_modifier=3, owner=user,
        )
        response = client.get(f'/api/spellbooks/characters/{char.id}/')
...
class TestSpellbookSpellManagement:
    ...
    def test_add_spell_to_spellbook(self, client, spellbook, spell_fireball):
        ...
        assert response.status_code == status.HTTP_201_CREATED
        assert PreparedSpell.objects.filter(
            spellbook=spellbook, spell=spell_fireball
        ).exists()

    def test_add_duplicate_spell_rejected(self, client, spellbook, spell_fireball):
        PreparedSpell.objects.create(
            spellbook=spellbook, spell=spell_fireball, prepared=False
        )
        data = {'spell_id': str(spell_fireball.id), 'prepared': False, 'notes': ''}
        response = client.post(f'/api/spellbooks/{spellbook.id}/add_spell/', data)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_remove_spell_from_spellbook(self, client, spellbook, spell_fireball):
        ...
        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not PreparedSpell.objects.filter(
            spellbook=spellbook, spell=spell_fireball
        ).exists()
    ...
    def test_remove_nonexistent_spell_returns_404(self, client, spellbook, spell_fireball):
        response = client.delete(
            f'/api/spellbooks/{spellbook.id}/remove_spell/?spell_id={spell_fireball.id}'
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_update_prepared_spell(self, client, spellbook, spell_fireball):
        ps = PreparedSpell.objects.create(
            spellbook=spellbook, spell=spell_fireball, prepared=False
        )
        response = client.patch(
...
class TestSpellbookActions:
    ...
    def test_copy_cost_with_scribes_discount(self, client, user, character):
        ...
        character.save()
        book = Spellbook.objects.create(
            name='Scribes Tome', owner=user, character=character
        )
        spell = Spell.objects.create(
...
class TestCalculateCopyCost:
    ...
    def test_order_of_scribes_50pct(self, user):
        char = Character.objects.create(
            name='Scribe', character_class='wizard', character_level=5,
            subclass='order_of_scribes', owner=user,
        )
        book = Spellbook.objects.create(name='Scribes', owner=user, character=char)
        spell = Spell.objects.create(
            name='Shield', level=1, school='abjuration',
            casting_time='1 action', range='Self', duration='1 round',
    ...
    def test_school_discount_applied(self, user):
        char = Character.objects.create(
            name='Evoker', character_class='wizard', character_level=5,
    ...
    def test_scribes_and_school_highest_wins(self, user):
        ...
        char = Character.objects.create(
            name='Power Scribe', character_class='wizard', character_level=10,
        ...
        spell = Spell.objects.create(
            ...
            casting_time='1 action', range='100 feet', duration='Instantaneous',
    ...
    def test_multiple_spells_summed(self, user):
        ...
        spells = [
            Spell.objects.create(
                name=f'Spell {i}', level=i, school='evocation',
                casting_time='1 action', range='90 feet', duration='Instantaneous',
                description='Spell',
            )
